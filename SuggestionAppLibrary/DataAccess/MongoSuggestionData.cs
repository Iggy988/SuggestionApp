...
public class MongoSuggestionData : ISuggestionData
{
    ...
    public async Task<List<SuggestionModel>> GetAllSuggestions()
    {
        ...
        if (output is null)
        {
            ...
            // ne zelimo da bude puno cached, posto je velika lista i brzo ce se mijenjati data
    ...
    {
        var output = await GetAllSuggestions();
        return output.Where(x => x.ApprovedForRelease == false && x.Rejected).ToList();
    }

    public async Task UpdateSuggestion(SuggestionModel suggestion)
    {
        await _suggestions.ReplaceOneAsync(s => s.Id == suggestion.Id, suggestion);
        //destroy cache, to get fresh wersionS
    ...
    public async Task UpvoteSuggestion(string suggestionId, string userId)
    {
        ...
        try
        {
            ...
            // replacing suggestion with new version that we just change
